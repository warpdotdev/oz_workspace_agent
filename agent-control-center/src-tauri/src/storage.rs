...
impl Storage {
    ...
    pub async fn new() -> StorageResult<Self> {
        ...
        info!("Storage initialized at {:?}", data_path);
    ...
    fn get_data_dir() -> StorageResult<PathBuf> {
        ...
        Ok(base_dir.join("agent-control-center"))
    }
    ...
    async fn persist(&self) -> StorageResult<()> {
        ...
        debug!("Data persisted to {:?}", self.data_path);
        Ok(())
    }
    ...
    pub async fn update_task(&self, task: Task) -> StorageResult<Task> {
        ...
        drop(data);
        self.persist().await?;
        info!("Updated task: {} ({})", task.title, task.id);
        Ok(task)
    }
    ...
    pub async fn reset(&self) -> StorageResult<()> {
        ...
        *data = StorageData::default();
        drop(data);
        self.persist().await?;
        info!("Storage reset to empty state");
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_agent_crud() {
        ...
        // Create
        let agent = Agent::new("Test Agent".to_string(), "crewai".to_string());
        let created = storage.create_agent(agent.clone()).await.unwrap();
        assert_eq!(created.name, "Test Agent");
        
        // Read
        let fetched = storage.get_agent(created.id).await.unwrap();
        assert_eq!(fetched.name, created.name);
        
        // Update
        let mut updated = fetched.clone();
        updated.status = AgentStatus::Running;
        let saved = storage.update_agent(updated).await.unwrap();
        assert_eq!(saved.status, AgentStatus::Running);
        
        // Delete
        storage.delete_agent(created.id).await.unwrap();
        assert!(storage.get_agent(created.id).await.is_err());
    }
}
