...
**Built with statistical rigor:**
- Train/test split (no in-sample fairy tales)
- P-value significance testing (p < 0.05 or it's noise)
- Multi-asset validation (edge must replicate)
...
5. **Ensemble** - Weighted combination of strategies by recent performance.
...
```bash
# Backtest all strategies on bitcoin (default: train/test split, 90 days)
python -m market_prediction.cli.main backtest --asset bitcoin

# Backtest specific strategy
python -m market_prediction.cli.main backtest --strategy momentum --days 120

# Multi-asset backtest (the real test - does edge replicate?)
python -m market_prediction.cli.main backtest --multi-asset

# Disable train/test split (not recommended - shows in-sample only)
python -m market_prediction.cli.main backtest --no-split
```
...
- **Directional Accuracy**: % of correct UP/DOWN predictions
  - Baseline (random): ~50%
  - Target: >55% with statistical significance
  
- **Statistical Significance**: p-value < 0.05
  - At n=100: need ~60% accuracy to be significant
  - At n=500: need ~54% accuracy to be significant
  - At n=1000: need ~53% accuracy to be significant
  
- **Magnitude Accuracy**: % of correct magnitude bucket predictions
  - Baseline (random): ~33%
  
- **Edge Replication**: Strategy must be significant across multiple assets
